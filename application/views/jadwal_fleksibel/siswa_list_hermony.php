...
<div class="flex max-w-[1280px] mx-auto w-full">
    ...
    <main class="flex-1 p-container-padding-mobile md:p-container-padding-desktop pb-24 lg:pb-container-padding-desktop">
        <div class="max-w-4xl mx-auto space-y-6">
            <!-- Header -->
            <div>
                <h2 class="text-2xl font-bold font-headline text-primary">Jadwal Kelas Fleksibel</h2>
                <p class="text-xs text-on-surface-variant mt-1">Daftar kelas pembelajaran online/hybrid terjadwal untuk Kelas <?= htmlspecialchars($siswa->nama_kelas) ?>.</p>
            </div>

            <!-- Banner Informasi Pola Jadwal -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <?php if (!empty($schedules)) : ?>
                        ...
                        <div class="lumina-card p-5 flex flex-col justify-between gap-4">
                            <div class="space-y-2">
                                <div class="flex justify-between items-center text-xs">
                                    ...
                                    <span class="font-semibold text-teal-600 flex items-center gap-1">
                                ...
                                <h4 class="font-headline font-bold text-base text-primary leading-snug">
                            ...
                            <div>
                                ...
                                if ($jenis === 'tugas') : 
                                ?>
                                    <span class="w-full block text-center py-2 bg-orange-100 text-orange-700 font-bold text-xs rounded-lg flex items-center justify-center gap-1 border border-orange-200">
                                        <span class="material-symbols-outlined text-sm">assignment</span> Tugas Mandiri
                                    </span>
                                <?php elseif ($jenis === 'online' || !empty($row->learning_link)) : ?>
                                    <?php if (!empty($row->learning_link)) : ?>
                                        <a href="<?= htmlspecialchars($row->learning_link) ?>" target="_blank" class="w-full block text-center py-2 bg-green-600 hover:bg-green-700 text-white font-bold text-xs rounded-lg transition-colors shadow-sm flex items-center justify-center gap-1">
                                            <span class="material-symbols-outlined text-sm">link</span> Masuk Kelas Online
                                        </a>
                                    <?php else : ?>
                                        <span class="w-full block text-center py-2 bg-green-100 text-green-700 font-bold text-xs rounded-lg flex items-center justify-center gap-1 border border-green-200">
                                            <span class="material-symbols-outlined text-sm">language</span> Kelas Online
                                        </span>
                                    <?php endif; ?>
                                <?php else : ?>
                                    <span class="w-full block text-center py-2 bg-gray-100 text-gray-700 font-bold text-xs rounded-lg flex items-center justify-center gap-1 border border-outline-variant">
                                        <span class="material-symbols-outlined text-sm">meeting_room</span> Kelas Offline
                                    </span>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else : ?>
                    <div class="col-span-2 lumina-card p-6 text-center text-on-surface-variant text-sm">Belum ada jadwal terdaftar untuk kelas Anda periode ini.</div>
                <?php endif; ?>
            </div>
        </div>
    </main>
