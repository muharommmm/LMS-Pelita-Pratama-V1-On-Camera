                        ...
                        <div class="col-8">
                            ...
                            <div class="row">
                                <div class="col-12 col-md-4 mb-3">
                                      ...
                                      <input type="hidden" name="id_materi" id="id-materi">
...
<script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    swal.fire("Sukses", "Nilai berhasil disimpan!", "success").then(() => {
                        $('#dropdown-materi').trigger('change');
    ...
    function createDropdownKelas(arrKelas) {
        ...
        if (arrKelas != null && arrKelas['1'] != null) {
            ...
            for (let j = 0; j < arrKelas['1'].length; j++) {
                if (kelas[arrKelas['1'][j]] !== undefined)
                    klsMateri.append('<option value="1-' + arrKelas['1'][j] + '">' + kelas[arrKelas['1'][j]] + '</option>');
            }
        } else {
        ...
        if (arrKelas != null && arrKelas['2'] != null) {
            ...
            for (let j = 0; j < arrKelas['2'].length; j++) {
                klsTugas.append('<option value="2-' + arrKelas['2'][j] + '">' + kelas[arrKelas['2'][j]] + '</option>');
            }
        } else {
            klsTugas.append('<option value="-">- -</option>');
        }
        var firstVal = $('#kelas-materi').val();
        if (firstVal && firstVal.indexOf('-') !== -1) {
            var type = firstVal.split('-')[0];
            label = (type === '1') ? 'Materi' : 'Tugas';
        }
    ...
    function createDropdownMateri(val) {
        if (!val || val === '-') return;
        var kelas = val;
        if (val.indexOf('-') !== -1) {
            kelas = val.split('-')[1];
        ...
        var textOpsi = item.kode + " - " + (item.judul_materi || "");
    ...
    function getLogSiswa() {
        ...
        var selKelasRaw = $('#kelas-materi').val();
        if (selKelasRaw === '-' || selKelasRaw == null) {
            $('#log').html('<tr><td>Tidak ada data</td></tr>');
            $('#table-info').addClass('d-none')
            return;
        }
        var selKelas = selKelasRaw;
        if (selKelasRaw && selKelasRaw.indexOf('-') !== -1) {
            selKelas = selKelasRaw.split('-')[1];
        }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var html = '<thead>' +
                        ...
                        '<th rowspan="2" class="text-center align-middle center" data-exclude="true" '+ styleHead +'>Hasil</th>' +
                    ...
                    $.each(resultAll, function (key, value) {
                        ...
                        html +=
                            ...
                            '<td class="align-middle text-center" data-exclude="true" >' +
