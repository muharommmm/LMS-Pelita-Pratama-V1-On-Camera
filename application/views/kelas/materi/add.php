                ...
                <div class="card-body">
                    ...
                    <div id="input-materi" class="row">
                        <div class="col-md-3 mb-3">
                            <label>Kode</label>
                            <input name="kode_materi" id="kode_materi" value="<?= $materi->kode_materi ?>" type="text"
                        ...
                        <div class='col-md-3 mb-3'>
                            ...
                            <div class='col-md-6 mb-3'>
                                <label>Tanggal Mulai <?= $jenis === '1' ? 'Materi' : 'Tugas' ?> <span class="text-danger">*</span></label>
                                <input type='text' id="tgl-mulai" name='tgl_mulai' value="<?= !empty($materi->tgl_mulai) ? $materi->tgl_mulai : date('Y-m-d H:i') ?>"
                                       class='tgl-datetime form-control form-control-sm' autocomplete='off' required/>
                            </div>
                            <div class='col-md-6 mb-3'>
                                <label>Batas Waktu (Deadline) <span class="text-danger">*</span></label>
                                <input type='text' id="deadline" name='deadline' value="<?= !empty($materi->deadline) ? $materi->deadline : date('Y-m-d H:i', strtotime('+3 days')) ?>"
                                       class='tgl-datetime form-control form-control-sm' autocomplete='off' required/>
                            </div>
...
<script>
    ...
    function onChangeMapel(mapel) {
        ...
        // Auto-generate code
        autoGenerateKode(mapel);
    }

    function autoGenerateKode(idMapel) {
        // Only auto-generate for new materials/tasks
        if ($('#id-materi').val() !== '') {
            return; 
        }
        if (!idMapel) {
        ...
        $.ajax({
            type: 'GET',
            url: base_url + 'kelasmateri/generate_kode_materi',
            data: {
                id_mapel: idMapel,
                id_guru: idGuru,
                jenis: jenis
            ...
            success: function(res) {
                if (res.status && res.kode) {
                    $('#kode_materi').val(res.kode);
                }
            }
        });
    }
